...
class Agent():
    ...
    def _package_action_into_msg(self,action):
        ...
        if((const.N_PORTS*2)+1 <= action <= const.N_PORTS*(2+const.N_ACCESSPOINTS)):
            ...
            if(self.verbose): print('HACKER: Trying to take over process {0} on port {1}...'.format(const.STRING_ACCESSPOINTS[accesspoint],const.STRING_PORTS[port]))
        ...
        if(const.N_PORTS*(2+const.N_ACCESSPOINTS)+1 <= action <= const.N_PORTS*(2+const.N_ACCESSPOINTS+const.N_VULNPARAMS)):
            ...
            if(self.verbose): print('HACKER: Trying to execute exploit with param {0} on port {1}...'.format(param,const.STRING_PORTS[port-1]))
